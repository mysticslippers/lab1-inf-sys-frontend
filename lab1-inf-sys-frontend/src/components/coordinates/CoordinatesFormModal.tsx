...
const CoordinatesFormModal: React.FC<Props> = ({
                                                   open,
                                                   initialItem,
                                                   onClose,
                                               }) => {
    ...
    const validate = (): boolean => {
        ...
        if (!x.trim()) {
            ...
            next.x = 'X должен быть числом (Double)';
        }
        ...
        if (!y.trim()) {
            ...
            const val = Number(y);
            if (Number.isNaN(val)) {
                next.y = 'Y должен быть числом (Float)';
            } else if (val <= -976) {
                next.y = 'Y должен быть больше -976';
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSubmitError(
                ...
                err?.message ??
                'Не удалось сохранить координаты'
            );
        } finally {
            setSubmitting(false);
        }
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70">
            <div className="relative w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-xl">
                ...
                <h3 className="mb-4 text-lg font-semibold">
                    {isEdit ? 'Редактировать координаты' : 'Создать координаты'}
                </h3>
...
export default CoordinatesFormModal;
